...
class FollowEndpoints[F[_]: Sync, Auth: JWTMacAlgo] extends Http4sDsl[F] {
...
object FollowEndpoints {
  def endpoints[F[_]: Sync, Auth: JWTMacAlgo](followService: FollowService[F], auth: SecuredRequestHandler[F, Long, User, AugmentedJWT[Auth, Long]]): HttpRoutes[F] =
    new FollowEndpoints[F, Auth].allEndpoints(followService, auth)
}
