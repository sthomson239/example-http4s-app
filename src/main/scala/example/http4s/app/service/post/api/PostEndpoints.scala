...
object PostEndpoints {
  def endpoints[F[_]: Sync, Auth: JWTMacAlgo](postService: PostService[F], auth: SecuredRequestHandler[F, Long, User, AugmentedJWT[Auth, Long]]): HttpRoutes[F] =
    new PostEndpoints[F, Auth].allEndpoints(postService, auth)
}
